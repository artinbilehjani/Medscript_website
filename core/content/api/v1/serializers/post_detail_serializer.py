...
class PostFileSerializer(serializers.ModelSerializer):
    ...
    def get_file_url(self, obj):
        """
        Direct URL to the file — used as the "open/view" link in the
        frontend (openBtn in post-detail.js). Opens in a new browser tab.
        nginx serves this directly from /media/ without hitting Django.
        """
        if not obj.file:
            return None
    ...
    def get_download_url(self, obj):
        """
        URL to the hardened download endpoint (PostFileDownloadAPIView in
        mediafiles/views.py). This goes through Django for auth/permission
        checks, then X-Accel-Redirect hands the actual byte transfer to
        nginx. Returns None when is_downloadable=False.
        """

...
(function () {
  ...
  async function loadPositions() {
    ...
    tbody.innerHTML = '<tr><td colspan="4" class="loading-row"><div class="spinner"></div></td></tr>';
    try { allPositions = await api('GET', '/positions/'); renderPositions(); }
    catch { tbody.innerHTML = '<tr><td colspan="4" style="color:var(--danger);text-align:center;padding:20px">Failed.</td></tr>'; }
  }

  function renderPositions() {
    const tbody = document.getElementById('positions-tbody');
    if (!allPositions.length) { tbody.innerHTML = '<tr><td colspan="4" style="color:var(--text-3);text-align:center;padding:30px">No positions yet.</td></tr>'; return; }
    tbody.innerHTML = allPositions.map(p => `
      <tr>
        <td><strong>${esc(p.name)}</strong></td>
        <td style="color:var(--text-2);font-size:13px">${esc(p.description || '—')}</td>
        <td style="font-family:'JetBrains Mono',monospace">${p.user_count}</td>
        <td><div class="action-btns">
          <button class="action-btn" title="Edit" onclick="window._editPos(${p.id})"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg></button>
          <button class="action-btn danger" title="Delete" onclick="window._deletePos(${p.id},'${esc(p.name)}')"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a1 1 0 0 1 1-1h4a1 1 0 0 1 1 1v2"/></svg></button>
        </div></td>
      </tr>`).join('');
  }

  document.getElementById('new-pos-btn')?.addEventListener('click', () => {
    document.getElementById('pos-id').value = '';
    document.getElementById('pos-form').reset();
    document.getElementById('pos-modal-title').textContent = 'New Position';
    openModal('pos-modal');
  });
  ...
  window._editPos = (id) => {
    ...
    if (!p) return;
    document.getElementById('pos-id').value       = p.id;
    document.getElementById('pos-name-inp').value = p.name;
    document.getElementById('pos-desc-inp').value = p.description || '';
    document.getElementById('pos-modal-title').textContent = 'Edit Position';
    openModal('pos-modal');
  };

  window._deletePos = (id, name) => {
    document.getElementById('confirm-text').textContent = `Delete position "${name}"? Users will lose this position.`;
    openModal('confirm-modal');
    document.getElementById('confirm-ok').onclick = async () => {
      closeModal('confirm-modal');
      try {
        await api('DELETE', `/positions/${id}/`);
        allPositions = allPositions.filter(p => p.id !== id);
        renderPositions();
        toast('Position deleted');
        if (_loaded['users']) populateUserPosSelect();
      } catch { toast('Delete failed', 'error'); }
    };
  };
  ...
  document.getElementById('pos-form')?.addEventListener('submit', async e => {
    ...
    const payload = { name: document.getElementById('pos-name-inp').value, description: document.getElementById('pos-desc-inp').value };
    try {
      if (id) {
        const updated = await api('PATCH', `/positions/${id}/`, payload);
        const idx = allPositions.findIndex(p => p.id === parseInt(id));
        if (idx !== -1) allPositions[idx] = { ...allPositions[idx], ...updated };
        toast('Position updated ✓');
      } else {
        allPositions.push(await api('POST', '/positions/', payload));
        toast('Position created ✓');
      }
      closeModal('pos-modal');
      renderPositions();
      populateUserPosSelect();
    } catch { toast('Save failed', 'error'); }
  });
  ...
  async function loadUsers() {
    ...
    try {
      [allUsers, allPositions] = await Promise.all([
        api('GET', '/users/'),
        allPositions.length ? Promise.resolve(allPositions) : api('GET', '/positions/'),
      ]);
      const badge = document.getElementById('nav-users-count');
      if (badge) badge.textContent = allUsers.length;
      renderUsers(allUsers);
    } catch {
      tbody.innerHTML = '<tr><td colspan="8" style="color:var(--danger);text-align:center;padding:20px">Failed.</td></tr>';
    }
  }

  function renderUsers(users) {
    const tbody     = document.getElementById('users-tbody');
    const typeLabel = { 1:'Customer', 2:'Admin' };
    const typeClass = { 1:'badge-customer', 2:'badge-admin' };
    if (!users.length) { tbody.innerHTML = '<tr><td colspan="8" style="color:var(--text-3);text-align:center;padding:30px">No users found.</td></tr>'; return; }
    tbody.innerHTML = users.map(u => `
      <tr>
        <td><strong>${esc(u.username)}</strong></td>
        <td style="color:var(--text-2)">${esc(u.display_name)}</td>
        <td style="font-size:12px;color:var(--text-3)">${esc(u.position_name || '—')}</td>
        <td><span class="badge ${typeClass[u.type] || ''}">${typeLabel[u.type] || u.type}</span></td>
        <td><span class="active-badge ${u.is_staff  ? 'on':'off'}">${u.is_staff  ? 'Yes':'No'}</span></td>
        <td><span class="active-badge ${u.is_active ? 'on':'off'}">${u.is_active ? 'Active':'Suspended'}</span></td>
        <td style="font-size:11px;color:var(--text-3);font-family:'JetBrains Mono',monospace">${fmt(u.created_date)}</td>
        <td><div class="action-btns"><button class="action-btn" title="Edit" onclick="window._editUser(${u.id})"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg></button></div></td>
      </tr>`).join('');
  }

  function filterUsers() {
    const q    = document.getElementById('users-search')?.value.toLowerCase() || '';
    const type = document.getElementById('users-type-filter')?.value || '';
    renderUsers(allUsers.filter(u =>
      (u.username.toLowerCase().includes(q) || (u.display_name||'').toLowerCase().includes(q)) &&
      (type === '' || String(u.type) === type)
    ));
  }
  document.getElementById('users-search')?.addEventListener('input', filterUsers);
  document.getElementById('users-type-filter')?.addEventListener('change', filterUsers);

  function populateUserPosSelect() {
    const sel = document.getElementById('user-pos-inp');
    if (!sel) return;
    sel.innerHTML = '<option value="">— No position —</option>' +
      allPositions.map(p => `<option value="${p.id}">${esc(p.name)}</option>`).join('');
  }
  ...
  window._editUser = (id) => {
    ...
    document.getElementById('user-pos-inp').value      = u.user_position || '';
    document.getElementById('user-type-inp').value     = u.type;
    document.getElementById('user-staff-inp').checked  = u.is_staff;
    document.getElementById('user-active-inp').checked = u.is_active;
    ...
    document.getElementById('user-modal-info').innerHTML = `
      ...
      <div><div class="user-modal-name">${esc(u.username)}</div><div class="user-modal-email">${esc(u.email || 'No email')}</div></div>`;
  ...
  document.getElementById('user-form')?.addEventListener('submit', async e => {
    ...
    const payload = {
      user_position: document.getElementById('user-pos-inp').value  || null,
      type:          parseInt(document.getElementById('user-type-inp').value),
      is_staff:      document.getElementById('user-staff-inp').checked,
      is_active:     document.getElementById('user-active-inp').checked,
    };
    try {

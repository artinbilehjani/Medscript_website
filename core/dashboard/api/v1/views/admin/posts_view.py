...
from ...serializers import PostEditorSerializer,PostFileAdminSerializer,CommentAdminSerializer
...
class PostEditorDetailView(APIView):
    ...
    def _get_post(self, pk):
        return get_object_or_404(
            Post.objects.prefetch_related('tag', 'category', 'files'),
            pk=pk
        )
    ...
    def patch(self, request, pk):
        ...
        post = self._get_post(pk)
        s = PostEditorSerializer(post, data=request.data, partial=True, context={'request': request})
...
class CommentListView(APIView):
    ...
    def get(self, request):
        ...
        qs = Comment.objects.select_related("author", "post", "parent").order_by("-created_date")
        if status_filter:
...
class CommentDetailView(APIView):
    ...
    def _obj(self, pk): return get_object_or_404(Comment, pk=pk)
